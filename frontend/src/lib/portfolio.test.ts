...
function transaction(side: 'buy' | 'sell', shares: number, price: number, fee = 0, date = '2026-09-24', tax = 0): PortfolioTransaction {
  return { id: `${side}-${shares}-${price}-${fee}-${tax}-${date}`, symbol: '2330.TWSE', name: '台積電', side, shares, price, fee, tax, date, createdAt: `${date}T00:00:00.000Z` }
}
...
describe('portfolio accounting', () => {
  ...
  it('rejects oversells, nonpositive prices, fractional shares, and invalid fees', () => {
    ...
    expect(() => createPortfolioTransaction({ symbol: '2330.TWSE', side: 'buy', shares: 1, price: 101, date: '2099-01-01' }, buys)).toThrow('晚於今日')
  })
})
