  ...
  const saveSnapshotMutation = useMutation({
    mutationFn: () => {
      ...
      return api.selectionReview.saveSnapshot({
        ...
        as_of_date: asOf,
        market_context_summary: `選股共 ${data.items.length} 檔標的，全市場覆蓋 ${data.coverage_info?.total_universe ?? '-'} 檔`,
        items: data.items.map((it: ScreenerResultItem, idx: number) => ({
          symbol: it.symbol,
          name: it.name,
          rank: idx + 1,
          ...
          fundamental_summary:
            it.revenue_yoy !== null && it.revenue_yoy !== undefined
              ? `營收年增 ${it.revenue_yoy.toFixed(1)}%`
              : null,
          chips_summary:
            it.foreign_net !== null && it.foreign_net !== undefined
              ? `外資買賣 ${(it.foreign_net / 1e3).toFixed(0)} 張`
              : null,
    ...
    onSuccess: () => {
      toast.success('已儲存本次選股快照！可至選股復盤追蹤表現')
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800/80">
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={saveSnapshotMutation.isPending || !data?.items?.length}
              className="inline-flex items-center gap-1.5 text-[11px] font-medium bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 px-2.5 py-1 rounded transition-colors disabled:opacity-50"
              title="將本次篩選結果儲存為不可變快照，以追蹤後續 1D/5D/20D 報酬表現"
            >
            ...
            <Link
              to="/selection-review"
              className="inline-flex items-center gap-1 text-[11px] text-zinc-400 hover:text-zinc-200 transition-colors ml-1"
            >
